...
cecho()
{
	...
	msg=
	help_info='cecho \
	-r -hl -re -t -t -t -t "this help page is generated by" -B -bl "cecho" -d -r -hl -re -t -t -t -t -d -n \
	-hl Usage: -d -n \
	...
	-t "-B | -blank" -t "-t | -tab" -t "-n | -newline" -d'
	...
	case $1 in
		...
		-h|-help|--help	)
			eval $help_info
			# TODO: the message will display twice without next empty command
			# i dont know why but fix it with a new empty cecho ...
			cecho -g -hl -re -t -t -t -t "the help info of this help page as an example" -t -t -t -t
			#cecho
			echo $help_info
		;;
